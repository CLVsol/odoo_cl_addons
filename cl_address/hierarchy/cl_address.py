...
class cl_address(models.Model):
    ...
    parent_id = fields.Many2one('cl_address', 'Parent Address', select=True, ondelete='restrict')
    complete_name = fields.Char(string='Full Address', compute='_name_get_fnc', store=False, readonly=True)
    child_ids = fields.One2many('cl_address', 'parent_id', 'Child Places')
    ...
    def name_get(self):
        ...
        if self._context.get('address_display') == 'short':
            return super(cl_address, self).name_get()
