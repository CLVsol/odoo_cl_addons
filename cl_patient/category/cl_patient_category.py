...
class cl_patient_category(models.Model):
    _name = 'cl_patient.category'

    name = fields.Char('Category', required=True, size=128, translate=True)
    parent_id = fields.Many2one('cl_patient.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('cl_patient.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(cl_patient_category, self).name_get()
