...
class cl_place_category(models.Model):
    _name = 'cl_place.category'

    name = fields.Char('Category', required=True, size=128, translate=True)
    parent_id = fields.Many2one('cl_place.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('cl_place.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(cl_place_category, self).name_get()
