...
class cl_annotation_category(models.Model):
    _name = 'cl_annotation.category'

    name = fields.Char('Category', required=True, size=128, translate=True)
    parent_id = fields.Many2one('cl_annotation.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('cl_annotation.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(cl_annotation_category, self).name_get()
