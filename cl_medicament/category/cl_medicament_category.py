...
class cl_medicament_category(models.Model):
    _name = 'cl_medicament.category'

    name = fields.Char('Category', required=True, size=128, translate=True)
    parent_id = fields.Many2one('cl_medicament.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('cl_medicament.category', 'parent_id', 'Child Categories')
    ...
    def name_get(self):
        ...
        if self._context.get('category_display') == 'short':
            return super(cl_medicament_category, self).name_get()
...
class cl_medicament(models.Model):
    _inherit = 'cl_medicament'

    category_ids = fields.Many2many(
        'cl_medicament.category',
        'cl_medicament_category_rel',
        'medicament_id',
        'category_id',
        'Categories'
