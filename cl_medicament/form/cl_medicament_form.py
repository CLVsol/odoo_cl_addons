...
class cl_medicament_form(models.Model):
    ...
    name = fields.Char(size=256, string='Medicament Form', required=True,
                       translate=True,
    ...
    active = fields.Boolean('Active',
                            ...
                            default=1)
    parent_id = fields.Many2one('cl_medicament.form', 'Parent Form', select=True, ondelete='restrict')
    complete_name = fields.Char(string='Full Medicament Form', compute='_name_get_fnc', store=False, readonly=True)
    child_ids = fields.One2many('cl_medicament.form', 'parent_id', 'Child Forms')
    parent_left = fields.Integer('Left parent', select=True)
    parent_right = fields.Integer('Right parent', select=True)

    _sql_constraints = [
        ('name_uniq', 'UNIQUE(name)', 'Medicament Form must be unique!'),
    ...
    def name_get(self):
        ...
        if self._context.get('form_display') == 'short':
            return super(cl_medicament_form, self).name_get()
